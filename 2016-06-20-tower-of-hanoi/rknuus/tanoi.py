...
class Tower(object):
    ...
    def __eq__(self, other):
        return (isinstance(other, self.__class__) and other._home == self._home and
                other._auxiliary == self._auxiliary and other._target == self._target)
    ...
    def __hash__(self):
        ...
        return hash('a'+str(self._home)) ^ hash('b'+str(self._auxiliary)) ^ hash('c'+str(self._target))
    ...
    def moves(self):
        ...
        if self._home:
            if Tower._can_move(source=self._home, target=self._auxiliary):
                tower = copy.deepcopy(self)
                tower._auxiliary.append(tower._home.pop())
                yield tower
            if Tower._can_move(source=self._home, target=self._target):
                ...
                yield tower
        if self._auxiliary:
            if Tower._can_move(source=self._auxiliary, target=self._home):
                tower = copy.deepcopy(self)
                tower._home.append(tower._auxiliary.pop())
                yield tower
            if Tower._can_move(source=self._auxiliary, target=self._target):
                tower = copy.deepcopy(self)
                tower._target.append(tower._auxiliary.pop())
                yield tower
        if self._target:
            if Tower._can_move(source=self._target, target=self._home):
                ...
                yield tower
            if Tower._can_move(source=self._target, target=self._auxiliary):
                tower = copy.deepcopy(self)
                tower._auxiliary.append(tower._target.pop())
                yield tower

    def is_solved(self):
        # Because the total number of disks remains constant, it's enough to
        # check whether home and interim are empty
        return (self._home == [] and self._auxiliary == [])
    ...
    def __str__(self):
        return '\n'.join(self._get_row(a, b, c) for a, b, c in
                         itertools.zip_longest(self._pad_stack(self._home),
                                               self._pad_stack(self._auxiliary),
    ...
    def transfer(self):
        ...
        while not queue.empty():
            ...
            for move in tower.moves():
                ...
                # that we already have seen an intermediate tower but got a shorter track now.
                if move not in seen:
